...
bool RegistryStore::RemoveProcHookEntry(DWORD pid, DWORD filetimeHi, DWORD filetimeLo, int hookId) {
	// Operate directly on raw REG_MULTI_SZ and parse numeric fields to match exactly PID+HI+LO+HOOKID.
	HKEY hKey = NULL;
	LONG ro = RegOpenKeyExW(HKEY_LOCAL_MACHINE, REG_PERSIST_SUBKEY, 0, KEY_READ | KEY_WRITE, &hKey);
	if (ro != ERROR_SUCCESS) return false;
	DWORD type = 0, dataSize = 0;
	LONG rq = RegQueryValueExW(hKey, PROCHOOK_VALUE_NAME, NULL, &type, NULL, &dataSize);
	if (rq != ERROR_SUCCESS || type != REG_MULTI_SZ || dataSize == 0) { RegCloseKey(hKey); return (rq == ERROR_FILE_NOT_FOUND) ? true : false; }
	std::vector<wchar_t> raw(dataSize/sizeof(wchar_t));
	rq = RegQueryValueExW(hKey, PROCHOOK_VALUE_NAME, NULL, NULL, reinterpret_cast<LPBYTE>(raw.data()), &dataSize);
	if (rq != ERROR_SUCCESS) { RegCloseKey(hKey); return false; }
	std::vector<wchar_t> out;
	bool removed = false;
	size_t i = 0, wcCount = raw.size();
	while (i < wcCount) {
		if (raw[i] == L'\0') { i++; continue; }
		const wchar_t* line = &raw[i];
		size_t len = wcslen(line);
		// Parse first four colon-delimited header fields
		DWORD lp=0,lhi=0,llo=0; unsigned int lhook=0;
		bool parseOk = false;
		do {
			const wchar_t* p = line; const wchar_t* colon = wcschr(p, L':'); if (!colon) break;
			std::wstring f1(p, colon - p);
			p = colon + 1; colon = wcschr(p, L':'); if (!colon) break; std::wstring f2(p, colon - p);
			p = colon + 1; colon = wcschr(p, L':'); if (!colon) break; std::wstring f3(p, colon - p);
			p = colon + 1; colon = wcschr(p, L':'); if (!colon) break; std::wstring f4(p, colon - p);
			// Convert hex fields to numbers
			lp = wcstoul(f1.c_str(), NULL, 16);
			lhi = wcstoul(f2.c_str(), NULL, 16);
			llo = wcstoul(f3.c_str(), NULL, 16);
			lhook = wcstoul(f4.c_str(), NULL, 16);
			parseOk = true;
		} while (0);
		if (parseOk && lp == pid && lhi == filetimeHi && llo == filetimeLo && (int)lhook == hookId) {
			removed = true; // skip this specific entry
		} else {
	...
	if (!removed) { RegCloseKey(hKey); return true; }
	if (out.empty() || out.back() != L'\0') out.push_back(L'\0');
	out.push_back(L'\0');
	LONG rw = RegSetValueExW(hKey, PROCHOOK_VALUE_NAME, 0, REG_MULTI_SZ, reinterpret_cast<const BYTE*>(out.data()), (DWORD)(out.size()*sizeof(wchar_t)));
	RegCloseKey(hKey);
	return rw == ERROR_SUCCESS;
}

bool RegistryStore::RemoveProcHookList(DWORD pid, DWORD filetimeHi, DWORD filetimeLo) {
	// Operate directly on raw REG_MULTI_SZ and remove all entries that match PID+HI+LO (parse header fields).
	HKEY hKey = NULL;
	LONG ro = RegOpenKeyExW(HKEY_LOCAL_MACHINE, REG_PERSIST_SUBKEY, 0, KEY_READ | KEY_WRITE, &hKey);
	if (ro != ERROR_SUCCESS) return false;
	DWORD type = 0, dataSize = 0;
	...
	while (i < wcCount) {
		...
		const wchar_t* line = &raw[i];
		size_t len = wcslen(line);
		// Parse first three colon-delimited header fields (PID,HI,LO)
		DWORD lp=0,lhi=0,llo=0;
		bool parseOk = false;
		do {
			const wchar_t* p = line; const wchar_t* colon = wcschr(p, L':'); if (!colon) break;
			std::wstring f1(p, colon - p);
		...
		if (parseOk && lp == pid && lhi == filetimeHi && llo == filetimeLo) {
			removedAny = true; // skip entries for this key
		} else {
			out.insert(out.end(), line, line + len);
			out.push_back(L'\0');
		}
	...
	if (!removedAny) { RegCloseKey(hKey); return true; }
	if (out.empty() || out.back() != L'\0') out.push_back(L'\0');
	out.push_back(L'\0');
	LONG rw = RegSetValueExW(hKey, PROCHOOK_VALUE_NAME, 0, REG_MULTI_SZ, reinterpret_cast<const BYTE*>(out.data()), (DWORD)(out.size()*sizeof(wchar_t)));
	RegCloseKey(hKey);
