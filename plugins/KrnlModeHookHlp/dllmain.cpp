	...
	void HandleApplySequence(HWND hwnd) {
		...
		for (const auto& entry : entries) {
			...
			if (addrOk) res.address = base + off;
				...
				MessageBoxW(hwnd, L"Failed to call ParseAddressText to convert string to hex value", L"Hook Sequence", MB_ICONERROR);
				return;
			}
			HookRow newRow;
			...
			if (ApplyHookEntry(ctx, entry, newRow, applyError)) {
				...
				MessageBoxW(hwnd, L"Apply Hook Failed", L"Hook Sequence", MB_OK | MB_ICONERROR);
