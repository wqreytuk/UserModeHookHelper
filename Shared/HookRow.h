...
namespace HookRowUtils {
    ...
    using PersistTuple = std::tuple<DWORD, DWORD, DWORD, int, DWORD, unsigned long long, unsigned long long, unsigned long long, std::wstring>;

    inline PersistTuple ToPersistTuple(const HookRow& r, DWORD pid, DWORD hi, DWORD lo) {
        return std::make_tuple(pid, hi, lo, r.id, r.ori_asm_code_len, r.ori_asm_code_addr, r.trampoline_pit, r.address, r.module);
    }
